...
"""
"""
使用此工具可根据位置和/或环境描述，从候选设备列表中筛选出目标设备。若没有具体的环境描述，请勿使用此工具。  

每次使用此工具仅针对一种设备类型。  

该工具的输入需为包含2个键的JSON字符串：  
- devices（GUID字符串列表，即全局唯一标识符字符串列表）  
- disambiguation_information（字符串类型）：设备周围的物体或环境描述，**请勿包含设备类型名称**。
"""
