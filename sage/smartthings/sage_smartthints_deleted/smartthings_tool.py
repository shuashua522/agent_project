...
"""
"""
使用此工具可查询不同设备的特定组件及功能对应的API文档。  
在调用API之前使用此工具，将提高你的操作成功率。  

该工具的输入内容需为一个包含字典列表的JSON字符串。  
列表中的每个字典均需包含以下两个键：  
- device_id（GUID字符串，即全局唯一标识符字符串）  
- capability_id（字符串类型）
"""
...
class SmartThingsPlannerToolConfig(BaseToolConfig):
    ...
    """
    该工具用于生成执行某条指令所需的 API 调用计划。该工具的输入为自然语言格式的用户指令。务必将原始用户指令提供给此工具，以确保其获取完整的上下文信息。
    """
