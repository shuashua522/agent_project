...
Use like python_interpreter(<python code string here>). Make sure the last line is the entrypoint to your code."""
    """
    必要时，使用此工具运行 Python 代码以实现任务自动化。
    使用方式如下：python_interpreter (<此处填入 Python 代码字符串>)。
    确保最后一行是你的代码的入口点。
    """
...
class ConditionCheckerToolConfig(BaseToolConfig):
    ...
    """
    使用此工具检查某个条件是否满足。该工具支持自然语言指令，会返回用于检查该条件的函数名称。
    输入内容应表述为问题形式。除需检查的问题外，你还应提供可能为该问题提供上下文的所有额外信息。
    """
    description: str = """
...
"""
"""
使用此工具在某个条件发生时获取通知。调用该工具时，应传入一个 JSON 字符串，其中包含以下键：
function_name（字符串）：用于检查条件的函数名称
notify_when（布尔值）：[true, false]
condition_description（字符串）
action_description（字符串）：当条件满足时要执行的操作
user_name（字符串）
"""
...
class NotifyOnConditionTool(SAGEBaseTool):
    ...
    def _run(self, command: str) -> str:
        ...
        # )
        print({"code": {fn_name: code_registry[fn_name]}, "condition": info})
