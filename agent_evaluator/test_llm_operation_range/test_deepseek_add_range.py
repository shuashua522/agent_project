...
def setup_logger():
    ...
    # 日志文件路径
    log_file = os.path.join(log_dir, "deepseek-add7.txt")
...
def llm_func(x,y):
    llm=get_llm()
    problem=f"计算：{x}*{y}。直接给出结果"
    # problem= utils.multiply_to_addition_str(x,y)
    # problem= utils.power_to_multiplication_str(base, exp)
    user_message = {
        "role": "user",
        "content": problem,
    }
